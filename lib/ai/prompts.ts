...
Also emit "annotations": the concrete, in-text markables a reader should see highlighted. Each "text" MUST be copied verbatim from the essay (so it can be located) and be short. Classify each as spelling, grammar, vocabulary (weak/imprecise word choice), or cohesion (linking/referencing). Give the corrected "fix" and a one-line "note". BE EXHAUSTIVE: mark EVERY distinct error you can quote — an error left unmarked is one the student repeats on exam day. A mid-band essay typically yields 15-30 annotations; only a genuinely clean essay yields few. Also mark UPGRADE opportunities as "vocabulary": correct-but-basic wording where a precise, natural collocation would lift Lexical Resource (put the stronger phrasing in "fix" and say in "note" that it is an upgrade, not an error). Cap at 30, prioritising errors over upgrades; never invent text that is not in the essay; annotations do NOT change the bands — score first, mark after. If the essay is a non-attempt or too short to mark, use an empty array.`;
...
const TASK2_CATEGORY_GUIDE: Record<Task2Category, string> = {
  ...
  advantages_disadvantages:
    'Advantages–disadvantages: describe ONE specific development, plan or practice, then ask "Do the advantages of this outweigh the disadvantages?" (the outweigh form — the candidate must weigh both sides and take a position).',
  positive_negative:
    'Positive/negative development: describe ONE current trend or change as a neutral statement, then ask "Do you think this is a positive or negative development?"',
};
...
export function buildGeneratePrompt(input: GenerateInput): AssembledPrompt {
  ...
  if (input.kind === "console_assistant") {
    ...
    const rules = [
      ...
      "⭐ NOT SEEING THE NUMBERS IS NOT A REASON TO WITHHOLD THE FILE. The snapshot carries no money at all, so you cannot say who owes what — and that is exactly when the debtors or ledger document is the useful answer. Say plainly that you cannot see the figures from here, AND offer the report that contains them. Never refuse a document because the data behind it is not in your snapshot; the file is generated from the database, not from you.",
  ...
  if (input.kind === "reading_set") {
    const system = [
      ...
      "LENGTH & STRUCTURE: the passage MUST be 750–900 words of developed academic prose (aim ~820). Give it a genuine expository or argumentative arc suited to the topic — weigh competing evidence, trace a history, explain a process, or state and qualify a position — with an opening that frames the topic, several middle paragraphs that EACH advance a DISTINCT point backed by specific detail (names, dates, figures, examples, contrasts), and a reflective close. Never a thin summary or a flat list of facts.",
      "ANGLE: if the spec names an 'angle', it is the REQUIRED lens for this passage — build the whole piece around that framing (e.g. a mechanism explainer, a chronological history, a debate between viewpoints, a single research study) rather than a generic overview of the topic. Keep it original and fully answerable from the passage alone.",
      "Number questions sequentially from 1 and use ONLY the question types named in the spec.",
      "If the spec includes a 'question_plan', it is AUTHORITATIVE: produce EXACTLY those question-type blocks, in that order, with those exact counts (it overrides any generic type list or total). Number sequentially from 1 across all blocks. A block annotated '[with a word bank A–J]' is a word-bank summary; one annotated '[as a flow-chart]' is a flow-chart note block; one annotated '[choose TWO letters]' is a two-answer multiple-choice pair — see those types below.",
      ...
      "PARAGRAPH LABELS: by DEFAULT the passage is one continuous story in ordinary paragraphs with NO letter labels, and NO question refers to a paragraph. The ONLY exception is when the question types include matching_information — then, and ONLY then, letter every paragraph 'A) ', 'B) ', 'C) '….",
      "matching_information (the 'Which paragraph contains the following information?' task): the passage MUST be 6–8 paragraphs, each prefixed 'A) ', 'B) '…. For each question 'prompt' is the piece of information to locate; 'options' is the list of paragraph letters present (e.g. ['A','B','C','D','E','F','G']); 'answer' is the single correct paragraph letter. The same letter MAY answer more than one question (you may use any letter more than once) and not every letter need be used.",
      "matching_features (the 'match each statement with the correct person' task): the passage MUST name 3–5 real-sounding people (researchers, historians, practitioners) and attribute a DISTINCT finding, claim or view to each, spread through the passage. 'options' is the SHARED list of those people's names exactly as the passage introduces them (e.g. 'Elena Ruiz'), the SAME list in the SAME order on EVERY question in the block (labelled A, B, C… in the UI). Each 'prompt' is a one-sentence statement PARAPHRASING something one person said/found — never a verbatim quote. 'answer' is the exact name from the list. The same person MAY answer more than one statement and not every person need be used.",
      ...
      "summary_completion WITH A WORD BANK (only when the question_plan marks the block '[with a word bank A–J]'): the answers are chosen from a SHARED list, not copied from the passage. Put in 'options' a list of 8–10 candidate words/short phrases — the SAME list on EVERY question in the block, in a fixed order (labelled A, B, C… in the UI) — with 2–3 MORE than there are gaps so some stay unused. Write each 'prompt' as a summary sentence with the gap as '______'. 'answer' is the EXACT text of the correct option from that list. Set 'word_limit' to null (there is no word limit — the student picks from the list). The summary must paraphrase the passage; the correct option is often a synonym of the passage wording, and distractors should be plausible but wrong.",
      "sentence_completion / summary_completion (no word bank) / note_completion: write the candidate-facing 'prompt' as the sentence or note line with the missing word(s) shown as a run of underscores '______' exactly where the answer goes (e.g. 'The research was funded by the ______ government.', or a note line like 'grows a thick ______ in winter'). The answer is the exact word(s) copied verbatim from the passage. Put the gap mid-line where natural, not always at the end. For these (NOT the word-bank summary) leave 'options' null.",
      ...
      "note_completion renders as a structured NOTES BOX, exactly like the real exam (a title, sub-headings, bullets, nested sub-points, and some lines with no blank). For EACH note_completion question also emit a 'note_meta' object: 'title' = the heading of the whole notes box (e.g. 'The saiga', 'Dar es Salaam's DART Bus Rapid Transit system'), the SAME on every line of the block; 'indent' = 0 for a top-level bullet or 1 for a sub-point/dash nested under the bullet above; 'before' = an array of CONTEXT lines (NOT answerable, no blank) shown immediately before this line, each { text, indent } — use it for a lead-in bullet that introduces sub-points (e.g. { text: 'has a large bulbous nose with swollen nostrils that', indent: 0 }) or a plain note with no blank that sits between gapped lines (e.g. { text: 'warm the air entering its lungs in winter', indent: 1 }).",
      "note_completion: keep each line short and telegraphic, not a full sentence. Use the 'section' field for the sub-heading inside the box (e.g. 'Adaptations', then 'Reasons for population decline'), reusing the same 'section' string for every line under it. Set both 'section' and 'note_meta' ONLY for note_completion; leave them null for every other type.",
      "note_completion marked '[as a flow-chart]' in the question_plan: the content is a STEP-BY-STEP PROCESS (a sequence of stages, e.g. a manufacturing method or experimental procedure), drawn as connected boxes. Write each line as ONE short stage in order; consecutive stages follow on from each other. 'note_meta.title' is the process caption (e.g. 'How sea salt is harvested'); use 'before' for a gap-less lead-in stage where one helps. Still emit note_meta exactly as for notes — the renderer adds the arrows.",
      "multiple_choice: options are the choices and answer is the exact text of the correct option; make distractors plausible.",
      "multiple_choice marked '[choose TWO letters]' in the question_plan (the 'Which TWO…?' task): produce EXACTLY 2 questions that share ONE stem and ONE option set — both questions carry the IDENTICAL 'prompt' (a question asking which TWO of the following…, e.g. 'Which TWO of the following points does the writer make about X?') and the IDENTICAL list of 5 'options'. The two questions' 'answer' fields are TWO DIFFERENT correct options (each the exact text of one option); the other three options are plausible but wrong. Each question keeps its own supporting_sentence proving ITS answer.",
...
const READING_SET_CONTRACT = `Emit ONE JSON object and nothing else — no prose, no code fence — exactly:
{
  ...
  "body": "<the full passage. DEFAULT: one continuous story in normal paragraphs with NO labels. ONLY when the question types include matching_information: write it as 6-8 paragraphs and prefix EACH with 'A) ', 'B) ', 'C) '… so questions can refer to a paragraph by letter>",
  "questions": [
    {
      ...
      "note_meta": { "title": "<notes-box / flow-chart title, same on every line of the block>", "indent": 0, "before": [ { "text": "<gap-less context line shown before this line>", "indent": 0 } ], "layout": "<'flowchart' for a flow-chart block, else 'notes' or null>" }
    }
  ]
}
Set "options" to null for types that have no options (true/false/not given, yes/no/not given, plain sentence/summary/note completion) — but a WORD-BANK summary ('[with a word bank A–J]') MUST put its shared A–J list in "options", matching_features puts its shared list of people in "options", and a '[choose TWO letters]' pair puts the same 5 choices in "options" on both questions. Set "word_limit" only for sentence/summary/note completion that draw from the passage (null for a word-bank summary). Set "section" and "note_meta" only for note_completion ("note_meta" null for all other types; "before" may be an empty array when the line has no preceding context; set "note_meta.layout" to "flowchart" only for a flow-chart block).`;

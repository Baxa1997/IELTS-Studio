...
export function buildGeneratePrompt(input: GenerateInput): AssembledPrompt {
  ...
  if (input.kind === "study_coach") {
    ...
    const rules = [
      ...
      "This product covers Writing, Reading and Listening in full, plus Speaking in beta (Part 2 cue-card practice — record an answer, get per-criterion grading). A full live speaking mock (Parts 1–3) is coming later — say so plainly if asked, and don't pretend features exist that don't.",

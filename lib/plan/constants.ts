/**
 * Study-plan shapes, form options, and pure helpers (level → task difficulty,
 * weekly quota, re-test cadence, countdown).
 *
 * Zero runtime dependencies on purpose: the onboarding form and the start wizard
 * are Client Components and import `SELF_REPORT_BANDS` / `pitchDifficulty` from
 * here, so they must not pull zod into the browser bundle. The validator lives in
 * `./types`, which re-exports this module.
 */

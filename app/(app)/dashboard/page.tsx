...
export default async function DashboardPage({
  ...
  /* Resolved ONCE and passed down. `getT()` reads the locale cookie, and these
     sub-components are plain functions rather than server components of their
     own, so a hook is not available to them and a second call would re-read the
     jar for the same request. */
  const t = await getT();
  ...
  const [
    { estimates, weakestCriterion, weakestReadingType, streakDays, history, recommendation },
    tasksThisWeek,
  ...
  const coachContext = [
    ...
    days != null && days >= 0
      ? `Test is ${days} day${days === 1 ? "" : "s"} away`
      : "No exam date set",
  ]
    .filter(Boolean)
    .join("\n");
  ...
  return (
    <div style={{ fontFamily: SANS, color: INK }}>
      ...
      <Header name={profile.full_name} plan={plan} days={days} t={t} />

      <div className="dash-grid" style={{ marginTop: 18 }}>
        {/* main column */}
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          <NextTask rec={recommendation} t={t} />
          <div className="dash-2">
            {SKILLS.map((s) => (
              <BandCard key={s} estimate={estimates.bySkill[s]} />
            ))}
          </div>
          <RecentResults history={history} t={t} />
        </div>

        {/* right rail: just the two things worth glancing at */}
        <aside style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          <WeekCard streakDays={streakDays} done={tasksThisWeek} goal={plan.weeklyGoal} t={t} />
          <FocusCard writing={weakestCriterion} reading={weakestReadingType} t={t} />
        </aside>
      </div>
  ...
  return (
    <div>
      ...
      <p style={{ fontFamily: SANS, fontSize: 15, color: MUTED, margin: "6px 0 0" }}>
        {t("dash.targetBand", { band: plan.targetBand.toFixed(1) })} ·{" "}
        {hasDate ? (
          t(days === 1 ? "dash.dayToTest" : "dash.daysToTest", { days: days! })
        ) : (
          <Link href="/plan" style={{ color: BRAND, fontWeight: 600, textDecoration: "none" }}>
            {t("dash.setExamDate")}
          </Link>
        )}
      </p>
...
function NextTask({ rec, t }: { rec: Recommendation; t: Translate }) {
  ...
  return (
    <div
      ...
      style={{
        position: "relative",
        overflow: "hidden",
        background: `linear-gradient(120deg,${HERO_A} 0%,${HERO_B} 62%,${HERO_C} 100%)`,
        borderRadius: 18,
        padding: "24px 26px",
        display: "flex",
        flexWrap: "wrap",
    ...
    >
      ...
      <div style={{ position: "relative", minWidth: 0, flex: "1 1 380px" }}>
        ...
        >
          <Sparkles size={13} strokeWidth={2.4} /> {t("dash.nextTask")}
        </div>
        <div
          style={{
            fontFamily: SERIF,
      ...
      >
        {[
          { Icon: CalendarClock, t: chipA },
          { Icon: TrendingUp, t: chipB },
        ].map(({ Icon, t }) => (
          <div
            key={t}
            style={{
  ...
  return (
    <div style={card}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <Eyebrow>{t("dash.thisWeek")}</Eyebrow>
        <span
          style={{ display: "inline-flex", alignItems: "center", gap: 6, color: AMBER }}
          title={t("dash.streak", { days: streakDays })}
        >
          ...
          >
            <span
              style={{
                ...
                background: d.filled ? BRAND : SURF,
                border: `1.5px solid ${d.today ? BRAND : d.filled ? BRAND : LINE}`,
              }}
            >
      ...
      >
        <span style={{ fontFamily: SANS, fontSize: 12.5, fontWeight: 600, color: MUTED }}>
          {t("dash.weeklyGoal")}
        </span>
        <span
          style={{
      ...
      <p style={{ fontFamily: SANS, fontSize: 12.5, color: MUTED, margin: "10px 0 0" }}>
        {met
          ? t("dash.goalReached")
          : t(streakDays > 0 ? "dash.moreToGoStreak" : "dash.moreToGo", { n: goal - done })}
      </p>
      <Link href="/plan" style={{ ...miniLink, marginTop: 10 }}>
        {t("dash.adjustPlan")} <ArrowRight size={14} />
      </Link>
    </div>
  );
}
...
function FocusCard({
  ...
  writing: WeakCriterion | null;
  reading: WeakReadingType | null;
  t: Translate;
}) {
  return (
    <div style={card}>
      <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
        ...
        <Eyebrow>{t("dash.focusAreas")}</Eyebrow>
      </div>
      <div style={{ display: "flex", flexDirection: "column", marginTop: 12 }}>
        <FocusRow
          Icon={PenLine}
          skill={t("nav.writing")}
          value={writing ? writing.label : t("dash.notMeasured")}
          sub={
            writing
              ? t("dash.avgBand", { band: writing.meanBand.toFixed(1) })
              : t("dash.gradeToFind")
          }
          href="/write"
        />
        ...
        <FocusRow
          Icon={BookOpen}
          skill={t("nav.reading")}
          value={reading ? reading.label : t("dash.notMeasured")}
          sub={
            reading
              ? t("dash.correctOf", {
                  pct: Math.round(reading.accuracy * 100),
  ...
  return (
    ...
    >
      ...
      <span style={{ flex: 1, minWidth: 0 }}>
        ...
        <span style={{ display: "block", fontFamily: SANS, fontSize: 12.5, color: MUTED }}>
          {sub}
        </span>
      </span>
...
function RecentResults({ history, t }: { history: HistoryEvent[]; t: Translate }) {
  return (
    <div>
      <div
        style={{
      ...
      >
        <h2 style={{ fontFamily: SERIF, fontWeight: 600, fontSize: 20, color: INK, margin: 0 }}>
          {t("dash.recentResults")}
        </h2>
        <Link href="/activities" style={{ ...miniLink }}>
          {t("dash.allActivities")} <ArrowRight size={14} />
        </Link>
      </div>
      {history.length === 0 ? (
        <div style={{ ...card, fontFamily: SANS, fontSize: 14.5, color: MUTED }}>
          {t("dash.noResults")}
        </div>
      ) : (
        <div style={{ ...card, padding: 0, overflow: "hidden" }}>
          {history.map((h, i) => (
            ...
            >
              <span style={{ fontFamily: SANS, fontWeight: 500, fontSize: 13.5, color: FAINT }}>
                {fmtDate(h.date)}
              </span>
              <span
                style={{
              ...
              <span style={{ display: "flex", alignItems: "center", gap: 12 }}>
                <DeltaBadge value={h.deltaVsPrev} t={t} />
                <span
                  style={{
                    fontFamily: SERIF,
                    fontWeight: 700,
...
function DeltaBadge({ value, t }: { value: number | null; t: Translate }) {
  ...
  if (value == null)
    return <span style={{ ...badge, color: FAINT, background: SURF }}>{t("dash.baseline")}</span>;
  if (value > 0)
    return (
      <span style={{ ...badge, color: EMERALD, background: SLATE_GREEN_BG }}>
        <TrendingUp size={12} /> {value.toFixed(1)}
      </span>
    );
  if (value < 0)
    return (
      <span style={{ ...badge, color: SLATE_RED, background: SLATE_RED_BG }}>
        <TrendingDown size={12} /> {Math.abs(value).toFixed(1)}
      </span>
    );
  return null;
}
...
function greeting(name: string | null, t: Translate): string {
  return name ? t("dash.welcomeName", { name: name.split(" ")[0] }) : t("dash.welcome");
}
...
function weekDots(streakDays: number): { label: string; today: boolean; filled: boolean }[] {
  ...
  return labels.map((label, i) => ({
    label,
    today: i === todayIdx,
    filled: i <= todayIdx && todayIdx - i < streakDays,
  }));
}

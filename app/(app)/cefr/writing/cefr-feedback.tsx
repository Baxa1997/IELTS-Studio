    ...
    <>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(380px,1fr))", gap: 14, marginTop: 18 }}>
        {CEFR_SUBSCALE_KEYS.map((key) => {
          ...
          return (
            <div key={key} style={{ background: "#fff", border: "1px solid #E6E8F0", borderRadius: 14, padding: "16px 18px" }}>
              ...
              <div style={{ marginTop: 9, height: 6, borderRadius: 3, background: "#EEF1F7", overflow: "hidden" }}>
              ...
              <p style={{ margin: "8px 0 0", fontSize: 13.5, lineHeight: 1.5, color: "#2C3247" }}>
                <strong style={{ color: est.color }}>Improve:</strong> {s.improve}
              </p>
            </div>
          );
        })}
      ...
      {/* Next level */}
      <div style={{ marginTop: 14, padding: "16px 18px", background: est.bg, border: `1px solid ${est.color}33`, borderRadius: 14 }}>
        <p style={{ margin: 0, fontSize: 13, fontWeight: 700, letterSpacing: ".03em", textTransform: "uppercase", color: est.color }}>To reach {grade.next_level.level}</p>
...
function ListCard({ title, color, items, icon }: { title: string; color: string; items: string[]; icon: React.ReactNode }) {
  return (
    <div style={{ background: "#fff", border: "1px solid #E6E8F0", borderRadius: 14, padding: "16px 18px" }}>

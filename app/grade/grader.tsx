...
export function PublicGrader() {
  ...
  return (
    <div ref={rootRef} style={{ fontFamily: SANS, scrollMarginTop: 76 }}>
      ...
      <section style={{ background: "#fff", border: `1px solid ${LINE}`, borderRadius: 18, boxShadow: "0 20px 44px -32px rgba(26,33,56,.35)" }}>
        ...
        <div style={{ padding: "16px 22px 14px", borderBottom: `1px solid ${SOFT_LINE}`, borderRadius: "18px 18px 0 0" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
            <span style={{ fontSize: 12.5, fontWeight: 800, letterSpacing: ".06em", color: "#8B919D", textTransform: "uppercase" }}>
              Question
            </span>
          ...
          <textarea
            ...
            style={{ display: "block", width: "100%", resize: "vertical", borderRadius: 10, border: "1px solid #E6E8EC", background: SOFT, padding: "10px 13px", fontFamily: SERIF, fontSize: 15, lineHeight: 1.55, color: "#3B4150" }}
          />
          <div style={{ display: "flex", flexWrap: "wrap", alignItems: "center", gap: 7, marginTop: 10 }}>
            <span style={{ fontSize: 12.5, color: "#8B919D", marginRight: 2 }}>Need one? Try a sample:</span>
            {PUBLIC_PROMPTS.map((p) => (
              <button
                key={p.id}
                type="button"
                onClick={() => setQuestion(p.prompt)}
                style={{ height: 28, padding: "0 12px", borderRadius: 999, fontFamily: "inherit", fontSize: 12.5, fontWeight: 700, cursor: "pointer", border: "1px solid #E2DED0", background: "#fff", color: "#3B4150" }}
              >
                {p.title}
              </button>
            ))}
            {question ? (
              <button
                type="button"
                onClick={() => setQuestion("")}
                style={{ height: 28, padding: "0 10px", borderRadius: 999, fontFamily: "inherit", fontSize: 12.5, fontWeight: 700, cursor: "pointer", border: "none", background: "transparent", color: "#8B919D" }}
              >
                Clear
              </button>
            ) : null}
        ...
        <textarea
          ...
          style={{ display: "block", width: "100%", resize: "vertical", minHeight: 300, border: "none", background: "#fff", padding: "20px 22px", fontFamily: SERIF, fontSize: 17, lineHeight: 1.9, color: "#3B4150" }}
        />
        ...
        <div style={{ position: "sticky", bottom: 0, zIndex: 5, display: "flex", flexWrap: "wrap", alignItems: "center", gap: 10, padding: "12px 16px", borderTop: `1px solid ${SOFT_LINE}`, background: "rgba(251,250,244,.94)", backdropFilter: "blur(6px)", WebkitBackdropFilter: "blur(6px)", borderRadius: "0 0 18px 18px" }}>
          ...
          <button
            ...
            style={{ display: "inline-flex", alignItems: "center", gap: 8, height: 42, padding: "0 15px", borderRadius: 10, border: "1px solid #E2DED0", background: "#fff", color: "#3B4150", fontFamily: "inherit", fontSize: 13.5, fontWeight: 700, cursor: reading ? "default" : "pointer", opacity: reading ? 0.6 : 1 }}
          >
            {reading ? (
              <>
          ...
          <span style={{ fontSize: 12.5, fontWeight: 700, fontVariantNumeric: "tabular-nums", color: tooLong ? RED : tooShort ? AMBER : "#8B919D" }}>
          ...
          <button
            ...
            style={{
              ...
              boxShadow: canGrade ? "0 6px 16px -6px rgba(125,1,50,.7)" : "none",
            }}
          >
            {status === "grading" ? (
              <>
      ...
      <p style={{ margin: "12px 2px 0", fontSize: 12.5, color: "#8B919D" }}>
        {gradesLeft > 0
...
function SignupWall({ onClose }: { onClose: () => void }) {
  ...
  return (
    ...
    >
      ...
      >
        <button type="button" onClick={onClose} aria-label="Close" style={{ position: "absolute", top: 12, right: 12, width: 32, height: 32, borderRadius: 9, border: "none", background: SOFT, color: "#8B919D", cursor: "pointer", fontSize: 17, lineHeight: 1 }}>×</button>
        <div style={{ width: 52, height: 52, margin: "0 auto", borderRadius: 14, background: "#FDF4F7", display: "flex", alignItems: "center", justifyContent: "center" }}>
        ...
        <div style={{ marginTop: 20, display: "flex", flexDirection: "column", gap: 10 }}>
          <Link href="/sign-in" style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", height: 48, borderRadius: 12, background: BRAND, color: "#fff", fontSize: 15.5, fontWeight: 700, textDecoration: "none", boxShadow: "0 6px 16px -6px rgba(125,1,50,.7)" }}>
            Create a free account
          </Link>
          <Link href="/sign-in" style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", height: 46, borderRadius: 12, background: "#fff", border: "1px solid #F0D3DE", color: BRAND, fontSize: 15, fontWeight: 700, textDecoration: "none" }}>
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", alignItems: "center", gap: 12, justifyContent: "space-between" }}>
        <button
          type="button"
          onClick={onAgain}
          style={{ display: "inline-flex", alignItems: "center", gap: 8, height: 42, padding: "0 18px", borderRadius: 11, border: "1px solid #E2DED0", background: "#fff", color: "#3B4150", fontFamily: "inherit", fontSize: 14, fontWeight: 700, cursor: "pointer" }}
        >
        ...
        <span style={{ fontSize: 12.5, fontWeight: 600, color: atLimit ? RED : "#8B919D" }}>
      ...
      <div style={{ background: "#fff", border: `1px solid ${LINE}`, borderRadius: 16, padding: "16px 20px", display: "flex", alignItems: "center", gap: 22, flexWrap: "wrap" }}>
        <div style={{ flex: "none", display: "flex", alignItems: "center", gap: 14 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
            <span style={{ fontSize: 12.5, fontWeight: 700, letterSpacing: ".04em", color: "#8B919D", textTransform: "uppercase", lineHeight: 1.1 }}>
              Overall<br />band
            </span>
      ...
      <div style={{ background: "#FDF4F7", border: "1px solid #F0D3DE", borderRadius: 16, padding: "18px 20px", display: "flex", flexWrap: "wrap", alignItems: "center", gap: 14, justifyContent: "space-between" }}>
        <div style={{ minWidth: 240, flex: "1 1 320px" }}>
          <p style={{ margin: 0, fontSize: 15, fontWeight: 800, color: INK }}>Go deeper — free account</p>
          <p style={{ margin: "5px 0 0", fontSize: 13.5, lineHeight: 1.55, color: "#3B4150" }}>
        ...
        <div style={{ display: "flex", gap: 9, flexWrap: "wrap" }}>
          <Link href="/sign-in" style={{ display: "inline-flex", alignItems: "center", height: 44, padding: "0 22px", borderRadius: 11, background: BRAND, color: "#fff", fontSize: 14.5, fontWeight: 700, textDecoration: "none", boxShadow: "0 6px 16px -6px rgba(125,1,50,.7)" }}>
            Create a free account
          </Link>
          <Link href="/sign-in" style={{ display: "inline-flex", alignItems: "center", height: 44, padding: "0 22px", borderRadius: 11, background: "#fff", border: "1px solid #F0D3DE", color: BRAND, fontSize: 14.5, fontWeight: 700, textDecoration: "none" }}>
            Sign in
          </Link>
        </div>
      </div>
...
const labelCap: React.CSSProperties = { fontSize: 11.5, fontWeight: 700, letterSpacing: ".04em", color: "#8B919D", textTransform: "uppercase" };
const detailP: React.CSSProperties = { margin: "4px 0 0", fontSize: 13.5, lineHeight: 1.5, color: "#3B4150" };
